...
function getAllTeddies() {
	//Création d'une requête avec l'objet de type XMLHttpRequest
	let request = new XMLHttpRequest();
	//L'objet 'this' de onreadystatechange permet d'accéder aux propriétés 'readyState' et 'status'
	request.onreadystatechange = function () {
		//Tant que l'état de la requête et le statut sont bons, envoi des données
		if (this.readyState == XMLHttpRequest.DONE && this.status == 200) {
			//création d'une variable 'response' (fichier responseText) et transformation du format Json en objet JavaScript
	...
	//Ouverture d'une connexion à un service web avec la méthode 'GET' pour récupérer l'URL 
